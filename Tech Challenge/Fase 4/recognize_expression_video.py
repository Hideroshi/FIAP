...
def detectar_faces(frame):
    ...
    return result, face_locations, face_encodings

# Nomeia as faces encontradas e conhecidas
def nomear_faces(known_face_encodings, known_face_names, face_encodings):
...
# Detecta emoções sobre cada frame encontrado e escreve
def detectar_emocoes(result, frame, face_locations, face_names):
...
def processar_video(cap, output_path):
    ...
    for _ in tqdm(range(total_frames), desc="Processando vídeo"):
        ...
        result, face_locations, face_encodings = detectar_faces(frame) # Detecta as faces no video 
        face_names = nomear_faces(known_face_encodings, known_face_names, face_encodings) # Nomeia as faces encontradas
        detectar_emocoes(result, frame, face_locations, face_names) # Detecta as emoções de cada frame
...
def main_loop(video_path, output_path):
...
# Roda o loop da função principal se rodar direto no arquivo 
if __name__ == "__main__":

    # Caminho para o arquivo de vídeo na mesma pasta do script
    base_dir = os.path.dirname(os.path.abspath(__file__))
    input_video_path = os.path.join(base_dir, 'video.mp4')  # Substitua 'meu_video.mp4' pelo nome do seu vídeo
    output_video_path = os.path.join(base_dir, 'output_video.mp4')  # Nome do vídeo de saída

    main_loop(input_video_path, output_video_path)